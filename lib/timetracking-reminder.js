...
export default {
  ...
  initEvents() {
    ...
    atom.project.onDidChangePaths((paths) => {
      ...
      if(this.lastRemindOnProjectChange && (new Date() - this.lastRemindOnProjectChange) < (1000 * atom.config.get('timetracking-reminder.pauseProjectChange'))) {
        return;
      }
    ...
    atom.getCurrentWindow().on('blur', () => {
      ...
      if(!atom.config.get('timetracking-reminder.remindOnInactivity')) {
        return;
      }
